...
//Hang Man
const themes = ["dh","food","body","clothes","verbs"]
const wordList = {
	dh: [
...
function hangmanCommand(item,sender) {
	if (word !== "") {
		play(item[0],sender)
		return;
	}
	switch (item[0]) {
		...
		case "score":
				if (!themes.includes(item[1])) {item[1] = "dh"}
				if (scores[item[1]][sender] !== undefined) {
					publishMessage(`<b>${sender}</b> has <b>${scores[item[1]][sender]} points</b>`)
				} else {
		...
		case "top":
			if (!themes.includes(item[1])) {item[1] = "dh"}
			let sortedScores;
			sortedScores = Object.entries(scores[item[1]]).sort((a, b) => b[1] - a[1]);
			const topFive = sortedScores.slice(0, 5);
			let tops = '<b>Top Scores:</b>';
			topFive.forEach(([player, points], index) => {
				tops += `<br>${index + 1}. <b>${player}:</b> ${points} points`;
			});
			if (!topFive.flat().includes(sender)) {
				tops += `<br> <b>${sender}:</b> ${points} points`
			}
			publishMessage(tops);
			break;
		//Temas disponíveis
		...
		case "restart":
			autoStart = !autoStart;
			publishMessage(`Autoreplay set to ${autoStart}`);
			break;
		default:
			publishMessage('Use !play to start');
			break;
	}
}

function newWord(newTheme) {
	if (!themes.includes(newTheme)) {newTheme = "dh"}
	theme = newTheme;
	word = wordList[theme][Math.floor(Math.random() * (wordList[theme].length + 1))]
	letters = word.length
	tries = 7
	guessedLetters = ''
...
function play(item,sender) {
	...
	if (item == 'hangman' || item == 'play' || item == 'word') {
            ...
            message: (messageEvent) => {
				if (messageEvent.message.sender !== 'Chat Bot') {
					if (messageEvent.message.description.startsWith("/")) {
						publishMessage(chatCommands(messageEvent.message.description.toLowerCase().slice(1)))
					} else if (messageEvent.message.description.startsWith("!")) {
						hangmanCommand(messageEvent.message.description.toLowerCase().slice(1).split(" "),messageEvent.message.sender)
					}
				};
            },
        };
...
function chatCommands(command) {
	...
	switch(command) {
		...
		case 'compare':
		case 'comparetool':
			response = 'https://dounford-felipe.github.io/DHM-Compare/'
			break;
		case 'chart':
		case 'graph':
		case 'stats':
			response = 'https://dounford-felipe.github.io/DHM-Compare/chart.html'
			break;
		case 'discord':
